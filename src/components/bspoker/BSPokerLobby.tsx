...
import { CreateBSRoomRequest, BSRoomListItem } from '../../types/bspoker.types'

interface BSPokerLobbyProps {
  onCreateRoom: (config: CreateBSRoomRequest) => Promise<void>
  onJoinRoom: (roomId: string, password: string, playerName: string) => Promise<void>
  onListRooms: () => Promise<BSRoomListItem[]>
  error: string | null
}

type LobbyView = 'menu' | 'create' | 'join' | 'list'

export const BSPokerLobby: React.FC<BSPokerLobbyProps> = ({
  onCreateRoom,
  onJoinRoom,
  onListRooms,
  error: propError
  ...
  const [publicRooms, setPublicRooms] = useState<BSRoomListItem[]>([])
  ...
  const [playerName, setPlayerName] = useState(() => {
    ...
    return localStorage.getItem('bspoker_player_name') || ''
  })
  ...
  const handleCreateRoom = async () => {
    ...
    try {
      const config: CreateBSRoomRequest = {
  ...
  const handleJoinRoom = async (roomId?: string) => {
    ...
    try {
      ...
      localStorage.setItem('bspoker_player_name', playerName.trim())
...
`}</pre>
      <div className="poker-cli-input-line">
        <span className="poker-cli-prompt">bspoker&gt;</span>
        <input
          type="text"
          value={playerName || ''}
          onChange={(e) => {
        ...
        <input
          ...
          onKeyDown={(e) => {
            if (e.key === 'Enter') {
              ...
              else if (parts[0] === 'players') setMaxPlayers(Math.min(8, Math.max(2, parseInt(parts[1]) || 6)))
...
`}</pre>
      <div className="poker-cli-input-line">
        ...
        <input
          ...
          className="poker-cli-input"
          placeholder="Enter command (e.g., 'room BSP-A4F9', 'join')"
          autoFocus
          disabled={loading}
        />
      </div>
...
Commands: back, refresh`
      ...
      output += `║  ID          Room Name         Players      Status          ║
╟───────────────────────────────────────────────────────────────╢
`
      publicRooms.forEach(room => {
        const id = room.roomId.padEnd(12)
        const name = (room.roomName + (room.hasPassword ? ' 🔒' : '')).padEnd(18)
        const players = `${room.currentPlayers}/${room.maxPlayers}`.padEnd(12)
        const status = room.status.padEnd(16)
        output += `║  ${id} ${name} ${players}${status}║\n`
      })
      output += `╚═══════════════════════════════════════════════════════════════╝
      ...
      <div className="poker-cli-container">
        ...
        <div className="poker-cli-input-line">
          ...
          <input
            ...
            className="poker-cli-input"
            placeholder="Enter command (e.g., 'join BSP-A4F9', 'refresh')"
            autoFocus
          />
        </div>
      </div>
